...
function Eff_dens( j, i)
  ...
  xzord = 0.25*(cord(j,i,1)+cord(j+1,i,1)+cord(j,i+1,1)+cord(j+1,i+1,1))
  press = 0
  do ii = 1, 4
...
!!$      Eff_dens = dens
      ...
      !    dens = dens * ( 1.-0.1*fmelt )
        if (zcord.ge.-6.e3.and.tmpr.ge.1000.and.xcord.lt.2.e3) then
        dens = dens -200.
        else
        dens = dens
        end if
...
222 continue ! don't understand !to let temp_li right

! HOOK
! Intrusions - melting effect - see user_ab.f90
!if( if_intrus .eq. 1 ) then ! what different between 222 to 222 ?
    HeatLatent = 500000.
